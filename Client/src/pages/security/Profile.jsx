        ...
        <div className="space-y-8">
          <Card className="p-10 text-center relative overflow-hidden group">
            <div className="w-36 h-36 rounded-3xl bg-indigo-600 mx-auto flex items-center justify-center text-white border-[6px] border-white shadow-2xl mb-6 relative group-hover:scale-105 transition-transform duration-500 overflow-hidden">
              <span className="text-5xl font-black">{(user?.name || 'S')[0]}</span>
            </div>
            <h2 className="text-2xl font-black text-slate-800 tracking-tight uppercase mb-1">{user?.name || 'Security Guard'}</h2>
            <p className="text-slate-400 font-bold text-xs uppercase tracking-widest mb-6">{user?.role || 'SECURITY'}</p>
            ...
            <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 flex items-center justify-center gap-6">
              <div className="text-center">
                ...
                <p className="font-black text-slate-700">Security</p>
              </div>
        ...
        <div className="lg:col-span-2 space-y-8">
          <Card className="p-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <Input label="Role" value={user?.role || 'SECURITY'} disabled={true} icon={Shield} />
            </div>
          </Card>
        </div>
      </div>
